...
import 'package:graville_operations/core/commons/widgets/workers_on_site_section.dart'; // ✅ new import
...
class _DashboardDetailScreenState extends State<DashboardDetailsScreen> {
  ...
  Color get _statusColor {
    switch (s.status) {
      case 'Completed': return const Color(0xFF1A9E5C);
      case 'Paused':    return const Color(0xFFDC2626);
      ...
      body: ListView(
        ...
        children: [
                    ...
                    Expanded(
                      child: Text(s.name,
                          style: const TextStyle(
                              color: Color(0xFF0F1117), fontSize: 18,
                    ...
                    Column(
                      ...
                      children: [
                        const Text('Total Contract Value',
                            style: TextStyle(
                                color: Color(0xFF7A7E8E), fontSize: 10, letterSpacing: 0.5)),
                        const SizedBox(height: 4),
                        Text('KES ${_fmt(s.totalAmount)}',
                            style: const TextStyle(
                                color: Color(0xFF0F1117), fontSize: 20,
          ...
          GridView.count(
            ...
            children: [
              _StatCard(label: 'Total Workers',     value: '${s.totalWorkers}', sub: 'Overall headcount', valueColor: const Color(0xFF1A5CFF)),
              _StatCard(label: 'Completion Rate',   value: '${s.progress}%',    sub: 'Overall progress',
                  valueColor: s.progress >= 80 ? const Color(0xFF1A9E5C) : s.progress >= 40 ? const Color(0xFFD97706) : const Color(0xFFDC2626)),
              _StatCard(label: 'Tasks Completed',   value: '$taskRate%',         sub: '${s.completedTasks} of ${s.tasks.length} tasks', valueColor: const Color(0xFF0F1117)),
              _StatCard(label: 'Contract Value',    value: 'KES ${_fmtShort(s.totalAmount)}', sub: 'Total budget', valueColor: const Color(0xFF0F1117)),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(child: _RingCard(label: 'Overall', sub: 'Completion',              percent: s.progress,   color: const Color(0xFF1A5CFF))),
              const SizedBox(width: 12),
              Expanded(child: _RingCard(label: 'Tasks',   sub: '${s.completedTasks}/${s.tasks.length} done', percent: taskRate, color: const Color(0xFF1A9E5C))),
              const SizedBox(width: 12),
              Expanded(child: _RingCard(label: 'Attendance', sub: 'This week',            percent: 0,            color: const Color(0xFFD97706))),
            ],
          ),
                    ...
                    Row(
                      children: [
                        Expanded(child: Text(t.name,
                            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF0F1117)))),
                        const SizedBox(width: 8),
                        Text('${t.percent}%',
                            style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF7A7E8E))),
                      ],
                    ),
          ...
          const SizedBox(height: 10),
          WorkersOnSiteSection(totalWorkers: s.totalWorkers),
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                        color: active ? const Color(0xFF1A5CFF) : Colors.black.withOpacity(0.1),
                      ),
                    ),
                    child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Text(gallery.photoLabels[i],
                            style: const TextStyle(fontSize: 10, color: Color(0xFF7A7E8E), fontWeight: FontWeight.w500),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      ...
      Expanded(child: Text(text,
          style: const TextStyle(fontSize: 12, color: Color(0xFF3A3D4A),
    ...
    child: Column(
      ...
      children: [
        ...
        Text(value, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700,
            color: valueColor, letterSpacing: -0.4),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      children: [
        ...
        Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF0F1117)),
            textAlign: TextAlign.center),
        const SizedBox(height: 2),
        Text(sub, style: const TextStyle(fontSize: 10, color: Color(0xFF7A7E8E)), textAlign: TextAlign.center),
      ],
    ),
  );
}
...
class _ReportItem extends StatelessWidget {
  ...
  Color get _typeColor => switch (report.type) {
    'weekly'   => const Color(0xFF1A9E5C),
    'incident' => const Color(0xFFDC2626),
    _          => const Color(0xFF1A5CFF),
  };

  Color get _typeBg => switch (report.type) {
    'weekly'   => const Color(0xFFE3F7ED),
    'incident' => const Color(0xFFFEE2E2),
    _          => const Color(0xFFE8EEFF),
  };
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(report.name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF0F1117))),
              const SizedBox(height: 2),
        ...
        const SizedBox(width: 8),
        IconButton(
          icon: const Icon(Icons.download_outlined, size: 18, color: Color(0xFF7A7E8E)),
          onPressed: () {},
